...
namespace AOTE_HORDE.Networking
{
	...
	// Network Manager Auth Functions
	public partial class PlayFabNetworkService : BaseNetworkService
	{
		...
		public void LoginInternal( Action<LoginResult> OnSuccess, Action<PlayFabError> OnFailed )
		{
			if ( AuthType.Equals(Authtypes.None) )
				AuthType = Authtypes.Silent;
			Authenticate(AuthType, OnSuccess, OnFailed);
		}
		...
		public void Authenticate( Authtypes authType, Action<LoginResult> OnSuccess, Action<PlayFabError> OnFailed )
		{
			AuthType = authType;
			Authenticate(OnSuccess, OnFailed);
		}

		/// <summary>
		/// Authenticate the user by the Auth Type that was defined.
		/// </summary>
		public void Authenticate( Action<LoginResult> OnSuccess, Action<PlayFabError> OnFailed )
		{
			...
			switch ( authType )
			{
				...
				case Authtypes.Silent:
					SilentlyAuthenticate(OnSuccess, OnFailed);
					break;

				case Authtypes.EmailAndPassword:
					AuthenticateEmailPassword(OnSuccess, OnFailed);
					break;
				case Authtypes.RegisterPlayFabAccount:
					AddAccountAndPassword(OnSuccess, OnFailed);
					break;
				case Authtypes.Steam:
					AuthenticateSteam();
					break;
				case Authtypes.Facebook:
					AuthenticateFacebook(OnSuccess, OnFailed);
					break;

				case Authtypes.Google:
					AuthenticateGooglePlayGames(OnSuccess, OnFailed);
					break;
		...
		/// </summary>
		private void AuthenticateEmailPassword( Action<LoginResult> OnSuccess, Action<PlayFabError> OnFailed )
		{
		...
		/// </summary>
		private void AddAccountAndPassword( Action<LoginResult> OnSuccess, Action<PlayFabError> OnFailed )
		{
			...
			SilentlyAuthenticate(( LoginResult result ) =>
			{
		...
		private void AuthenticateFacebook( Action<LoginResult> OnSuccess, Action<PlayFabError> OnFailed )
		{
#if FACEBOOK
		...
		private void AuthenticateGooglePlayGames( Action<LoginResult> OnSuccess, Action<PlayFabError> OnFailed )
		{
#if GOOGLEGAMES
		...
		private void SilentlyAuthenticate( Action<LoginResult> OnSuccess, Action<PlayFabError> OnFailed )
		{
			...
			{
				Debug.Log("Playfab Login Failed");
				//report errro back to the subscriber
				if ( OnFailed != null )
				{
					//make sure the loop completes, callback with null
		...
		public void UnlinkSilentAuth( Action OnSuccess, Action<PlayFabError> OnFailed )
		{
			SilentlyAuthenticate(( LoginResult result ) =>
			{
#if UNITY_ANDROID && !UNITY_EDITOR
